...
extension (rule: Rule):
  def reverse: Rule = rule.foldLeft(
    ...
    case (map, (k, v)) => v.foldLeft(map):
      case (map1, (k1, n)) =>
        map1 + (k1 -> (map1(k1) + (k -> n)))

  def reachableFrom(from: String): Set[String] =
    val reversed = reverse
    def loop(visited: Set[String], toVisit: List[String]): Set[String] =
      toVisit match
        case Nil =>
          visited
        case current :: remainder if visited `contains` current =>
          loop(visited, remainder)
        case current :: remainder =>
          val nexts = reversed(current).keySet -- visited
          loop(visited + current, nexts.toList ::: remainder)
    loop(Set.empty, from :: Nil)

  def countInner(from: String): Int =
    def loop(cache: Map[String, Int], from: String): Map[String, Int] =
      if cache.contains(from) then cache
      else if rule(from).isEmpty then cache + (from -> 0)
      else
        val children = rule(from)
        val cache1 = children.keySet.foldLeft(cache)(loop)
        val count = children.map{ case (k, v) => (cache1(k) + 1) * v}.sum
        cache1 + (from -> count)
    loop(Map.empty, from)(from)

def parseRule(s: String): Rule = s.split("\n").map:
  (line) =>
    def removeLastWord(s: String): String = s.trim.split(" ").init.mkString(" ")
    val Array(subj, obj) = line `split` "contain"
    val key = removeLastWord(subj)
    val valueMap = obj.trim.init match
      case "no other bags" => Map.empty
      case bags =>
        bags.split(", ").map:
          (bagString) =>
            val firstSpace = bagString.indexOf(" ")
            val (count, color) = bagString.splitAt(firstSpace)
            removeLastWord(color) -> count.toInt
        .toMap
    key -> valueMap
.toMap

@main def part1: Unit =
  val ans = parseRule(input).reachableFrom("shiny gold").size - 1
  println(ans)

@main def part2: Unit =
  val ans = parseRule(input).countInner("shiny gold")
  println(ans)

lazy val input = """dull silver bags contain 2 striped magenta bags, 2 dark coral bags, 1 bright orange bag, 4 plaid blue bags.
