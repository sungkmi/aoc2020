...
extension (p: Password)
  ...
  def isValid2: Boolean =
    (p.value.size > (p.min `max` p.max) - 1) && {
        val set = Set(p.value(p.min - 1), p.value(p.max - 1))
        set.size == 2 && set.contains(p.char)
    }

def parse(line: String): Password =
  val Array(cond, value) = line `split` ": "
  val Array(range, char) = cond `split` " "
  val Array(min, max) = range `split` "-"
  Password(min.toInt, max.toInt, char.head, value)

@main def part1: Unit = {
    val ans = xs.map(parse).filter(_.isValid)
    println(ans.size)
}

@main def part2: Unit = {
    val ans = xs.map(parse).filter(_.isValid2)
    println(ans.size)
}

lazy val xs = input.lines.iterator().asScala.toVector

lazy val input = """6-10 p: ctpppjmdpppppp
